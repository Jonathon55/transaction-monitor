...
export const createEdge = async (
  ...
  try {
    ...
    if (result.records.length === 0) {
      console.error(
        `Transaction creation failed: Could not find businesses with IDs ${from} and/or ${to}`
      );
      return null as any;
    }
  ...
  try {
    ...
    return result.records.map((record, index) => ({
      ...
      transactionCount: record.get('transactionCount')
        ? parseInt(record.get('transactionCount'), 10)
        : 0,
      transactionAmount: record.get('transactionAmount')
        ? parseFloat(record.get('transactionAmount'))
        : 0,
    }));
  } catch (error) {
